"""Stage 2: read cached OOF predictions, run bootstrap + conformal-under-shift,
write report_<tag>.json. Logic identical to analyze_qwen7b.py.
Usage: python3 stage2_report.py <tag>
"""
import json, os, sys
import numpy as np
...
out={"model":TAG,"answered":int(len(y)),"halluc_rate":float(y.mean()),
     "auc_retrieval":float(roc_auc_score(y,p_ret)),
     "auc_model":float(roc_auc_score(y,p_mod)),
     "auc_combined":float(roc_auc_score(y,p_all)),
